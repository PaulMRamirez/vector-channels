...
| Flow | One continuous variable (rate) | Animated chevrons on the path; heading and steady motion show direction, opacity shows rate |
...
1. **Rate drives opacity; motion is a constant stream.** Chevrons are spaced evenly in physical arc length and all advance at one constant screen speed, so density stays uniform along the path. The rate variable sets each chevron's **opacity** — full where the rover is moving, fading to nothing where it holds station. Heading shows direction; a chevron crossing a stationary stretch simply fades out and back in.
2. **Why not speed-encodes-rate.** The first implementation warped speed by rate (`u = integral(ds / rate)`, chevrons even in `u`, so they moved fast on high-rate stretches and stalled on stationary ones). It read as honest — until we ran the demo data through it and measured the result: **~60 of 65 chevrons piled into the frozen stations, leaving the drive legs nearly bare.** Even spacing in the warped coordinate concentrates particles wherever flow is slow — that *is* the correct steady-state density (like leaves in an eddy), but it inverts the intended "streaming along the traverse" read, and it can't be tuned out: any rate floor high enough to un-pile the chevrons is also high enough that stations no longer freeze. The lesson (recorded because it nearly shipped): a physically-honest model is not automatically the right *encoding* — verify the visual distribution on real data, not just the local behavior. The streaming model trades speed-encodes-rate for opacity-encodes-rate and keeps uniform density.
3. **Stateless in time.** Chevron position is a pure function of `RenderInput.timeMs` — `(k·spacing + c·t) mod totalLen` — so `draw()` keeps no cross-frame particle state, consistent with the renderer's cache-only model, and is deterministic (testable, resume-safe).
4. **The animation loop lives in the host, not `core`.** The Leaflet layer runs a continuous `requestAnimationFrame` loop only while a flow variable is assigned, feeding elapsed time in; `_scheduleRedraw` yields to it so frames aren't double-painted. `core` stays a pure "draw one frame at time t" surface.
5. **Reduced motion is a first-class fallback.** Under `prefers-reduced-motion` the host does not start the loop and passes no time; chevrons render static, still showing heading and rate. Motion is an enhancement, not the only carrier.
6. **Independent of the other roles (principle 1).** Chevrons ride the path centerline (no new geometry), draw above the strip and below event glyphs, and — deliberately — are *not* coupled to the uncertainty fade. The prototype briefly faded chevrons over uncertain data as a "cooperation" nicety; that was dropped to keep the roles orthogonal. Flow replaces nothing.
